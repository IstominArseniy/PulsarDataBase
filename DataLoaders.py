...
def load_MeerKAT_mean_data(file_name):
    ...
    for channel in range(8):
        ...
        profile_data[1] += weight * (np.cos(2 * beta) * Q - np.sin(2 * beta) * U)
        profile_data[2] += weight * (np.sin(2 * beta) * Q + np.cos(2 * beta) * U)
    ...
    #----------L, PA------------------
    profile_data[4] = np.sqrt(profile_data[1]**2 + profile_data[2]**2) - aux.noise_mean(np.sqrt(profile_data[1]**2 + profile_data[2]**2))
    #TODO better noise bias elimination
    profile_data[5] = aux.shift_angle(0.5 * np.arctan2(profile_data[2], profile_data[1])) * 180 / np.pi
    # profile_data[5] = aux.shift_angle(0.5 * np.arctan2(sinPA, cosPA)) * 180 / np.pi
    return profile_data
